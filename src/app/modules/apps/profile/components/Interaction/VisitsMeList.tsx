...
import {getVisitMeList} from '../../../../../../API/api-endpoint'
...
const VisitsMeList = ({handleChange}) => {
  let location = useLocation()
  let userId = GetIDFromURL(location)

  const [pageSize, setPageSize] = useState(10)
  const [totalPage, setTotalPage] = useState(0)
  const [VisitsMeList, setVisitsMeList] = useState([])

  const page = 1

  useEffect(() => {
    getAllVisitsMeList(page, pageSize)
  }, [])

  const getAllVisitsMeList = async (page: any, pageSize: any) => {
    let result = await getVisitMeList(userId, page, pageSize)
    setVisitsMeList(result.data)
          ...
          <div className='table-responsive'>
            ...
            >
              ...
              <tbody className='text-gray-600 '>
                {VisitsMeList !== undefined &&
                  VisitsMeList.map((item: any, index: any) => {
                    return (
                      <tr key={index}>
                        <td>
