...
import {getDisLikeMeList} from '../../../../../../API/api-endpoint'
...
const DisLikesMeList = ({handleChange}) => {
  let location = useLocation()
  let userId = GetIDFromURL(location)

  const [pageSize, setPageSize] = useState(10)
  const [totalPage, setTotalPage] = useState(0)
  const [disLikesMeList, setDisLikesMeList] = useState([])

  const page = 1

  useEffect(() => {
    getAllDisLikesMeList(page, pageSize)
  }, [])

  const getAllDisLikesMeList = async (page: any, pageSize: any) => {
    let result = await getDisLikeMeList(userId, page, pageSize)
    setDisLikesMeList(result.data)
          ...
          <div className='table-responsive'>
            ...
            >
              ...
              <tbody className='text-gray-600 '>
                {disLikesMeList !== undefined &&
                  disLikesMeList.map((item: any, index: any) => {
                    return (
                      <tr key={index}>
                        <td>
