...
const ChatInner = (props: any) => {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      if (message.trim().length !== 0) {
        ws.send(
          JSON.stringify({
            message: message,
            senderId: currentUserType === 'Normal' ? receiverUserDetails.userId : currentUserId,
            receiverId: currentUserType === 'Normal' ? currentUserId : receiverUserDetails.userId,
            type: 'text',
            chatRoomId: receiverUserDetails.chatRoomId,
  ...
  const updateChatmember = async (action: string) => {
    ...
    let result = await pinOrLikeChatMember(
      currentUserType === 'Normal' ? currentUserId : receiverUserDetails.userId,
      receiverUserDetails?.chatRoomId,
              ...
              <div>
                ...
                <p className='fs-8'>
                  {currentUserType === 'Normal'
                    ...
                    : `${CurrentUser?.fullName} Message to ${receiverUserDetails?.fullName}`}
                </p>
              </div>
